class Player {
    ...
    useItem(item) {
        if(item.health) {
            ...
            console.log(`Player ${this.name} recieved ${item.health} health points`)
        }
        if(item.power) {
            this.power += item.power
            console.log(`Player ${this.name} recieved ${item.power} power points`)
        }
    }
}
...
class ShootingGame {
    ...
    start() {
        ...
        if (this.player1.health <= 0) {
            ...
            console.log(`This winner is ${this.player1.name}`)
        }
